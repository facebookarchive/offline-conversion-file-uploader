...
|-------|-----------|--------------------------------------|
|upload|Hash and upload the CSV file to Facebook.|**`accessToken`** **`apiVersion`** **`configFilePath`** **`dataSetID`** **`inputFilePath`** **`mapping`** `batchSize` `customTypeInfo`  `delimiter` `format` `header` `ignoreSampleErrors` `logging` `namespaceID` `presetValues` `reportOutputPath` `skipRowsAlreadyUploaded` `uploadID` `uploadTag` `uploadTagPrefix`|
|validate|Do a dry-run and validate some sample rows.|**`accessToken`** **`apiVersion`** **`configFilePath`** **`dataSetID`** **`inputFilePath`** **`mapping`** `customTypeInfo` `delimiter` `format` `header` `logging` `namespaceID` `numRowsToValidate` `presetValues` `reportOutputPath`|
...
|upload-preprocessed|Upload the hashed CSV generated by `preprocess` command.|**`accessToken`** **`apiVersion`** **`dataSetID`** **`inputFilePath`** `batchSize` `configFilePath` `ignoreSampleErrors` `logging` `namespaceID` `reportOutputPath` `skipRowsAlreadyUploaded` `uploadID` `uploadTag` `uploadTagPrefix`|
